...
const generateNotifications = (): Notification[] => {
  ...
  // Notificación para medicación próxima
  notifications.push({
    id: 'notif-1',
    type: 'medication',
  ...
  // Notificación de medicación perdida
  notifications.push({
    id: 'notif-2',
    type: 'warning',
...
export default function MyTreatments() {
    ...
    treatments.forEach(treatment => {
      if (treatment.status === 'Active') {
        treatment.schedule.forEach(time => {
          ...
          // Simular estado: tomado si la hora ya pasó (70% probabilidad), pendiente si no
  ...
  const handleMedicationSkipped = (scheduleId: string, reason: string = 'Patient choice') => {
    ...
    // Crear notificación de medicación saltada
    const newNotification: Notification = {
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-800">
                      {todaySchedule.filter(item => item.taken).length}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-lg shadow border-l-4 border-blue-500">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-800">
                      {todaySchedule.filter(item => !item.taken && !item.skipped).length}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-4 rounded-lg shadow border-l-4 border-red-500">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-gray-800">
                      {todaySchedule.filter(item => item.skipped).length}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
                ...
                {treatments.map((treatment) => (
                  <div key={treatment.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-medium text-gray-800">{treatment.medication}</h4>
                    ...
                    <p className="text-sm text-gray-600 mb-1">{treatment.dosage} • {treatment.frequency}</p>
                    <p className="text-xs text-gray-500">Prescribed by {treatment.prescribedBy}</p>
                    {treatment.notes && (
                      <p className="text-xs text-blue-600 mt-2 italic">{treatment.notes}</p>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <p className="text-sm text-red-700 mb-2">
                If you experience severe side effects or need immediate medical attention:
              </p>
